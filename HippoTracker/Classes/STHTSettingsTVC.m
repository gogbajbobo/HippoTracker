...
@interface STHTSettingsTVC () <UITextFieldDelegate, NSFetchedResultsControllerDelegate>
...
@interface STHTSettingsTableViewCell ()

@end


@implementation STHTSettingsTableViewCell
...
@implementation STHTSettingsTVC

#pragma mark - STHTSettingsTVC
...
//    [locationTrackerSettings addObject:@[@"slider", @"0", @"24", @"0.5", @"locationTrackerFinishTime"]];
    ...
    [locationTrackerSettings addObject:@[@"slider", @"10", @"500", @"50", @"HTCheckpointInterval"]];
...
//    NSArray *groupNames = [NSArray arrayWithObjects:@"general", @"location", @"battery", @"map", @"syncer", nil];
    NSArray *groupNames = [NSArray arrayWithObjects:@"location", @"syncer", nil];
...
- (NSDictionary *)controlsSettings {
    if (!_controlsSettings) {
        _controlsSettings = [STHTSettingsTVC controlsSettings];
    }
...
- (NSString *)formatValue:(NSString *)valueString forSettingName:(NSString *)settingName{
    ...
    } else if ([settingName isEqualToString:@"trackScale"] || [settingName isEqualToString:@"HTSlowdownValue"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    STHTSettingsTableViewCell *cell = nil;
    
    NSString *controlType = [self controlTypeForIndexPath:indexPath];
    
    if ([controlType isEqualToString:@"slider"]) {
        cell = [[STHTSettingsTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:cellIdentifier];
        [self addSliderToCell:cell atIndexPath:indexPath];
        
    } else {
        cell = [[STHTSettingsTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
...
- (void)addSliderToCell:(STHTSettingsTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
...
- (void)addSwitchToCell:(STHTSettingsTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
...
- (void)addTextFieldToCell:(STHTSettingsTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
...
- (void)addSegmentedControlToCell:(STHTSettingsTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
...
- (void)settingsChanged:(NSNotification *)notification {
    ...
    STHTSettingsTableViewCell *cell = (STHTSettingsTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPath];
...
- (void)sliderValueChanged:(UISlider *)slider {
    ...
    STHTSettingsTableViewCell *cell = (STHTSettingsTableViewCell *)slider.superview.superview;
...
- (void)sliderValueChangeFinished:(UISlider *)slider {
    NSIndexPath *indexPath = [self.tableView indexPathForCell:(STHTSettingsTableViewCell *)slider.superview.superview];
    NSString *settingName = [self settingNameForIndexPath:indexPath];
    NSString *value = [NSString stringWithFormat:@"%f", slider.value];
    if ([settingName isEqualToString:@"desiredAccuracy"]) {
        value = [self desiredAccuracyValueFrom:rint(slider.value)];
...
- (void)switchValueChanged:(UISwitch *)senderSwitch {
    STHTSettingsTableViewCell *cell = (STHTSettingsTableViewCell *)senderSwitch.superview.superview;
    NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
    NSString *settingName = [self settingNameForIndexPath:indexPath];
...
- (void)segmentedControlValueChanged:(UISegmentedControl *)segmentedControl {
    STHTSettingsTableViewCell *cell = (STHTSettingsTableViewCell *)segmentedControl.superview.superview;
    NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
    NSString *settingName = [self settingNameForIndexPath:indexPath];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    STHTSettingsTableViewCell *cell = (STHTSettingsTableViewCell *)textField.superview.superview;
    NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
    NSString *settingName = [self settingNameForIndexPath:indexPath];
    NSString *groupName = [[self groupNames] objectAtIndex:indexPath.section];
