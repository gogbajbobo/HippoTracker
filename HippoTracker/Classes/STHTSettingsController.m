...
@interface STHTSettingsController() <NSFetchedResultsControllerDelegate>
...
+ (STHTSettingsController *)initWithSettings:(NSDictionary *)startSettings {
    STHTSettingsController *settingsController = [[STHTSettingsController alloc] init];
...
- (NSMutableDictionary *)currentSettingsForGroup:(NSString *)group {
    ...
    for (STHTSettings *setting in groupSettings) {
...
- (void)checkSettings {
    NSDictionary *defaultSettings = [STHTSettingsController defaultSettings];
    ...
    for (NSString *settingsGroupName in [defaultSettings allKeys]) {
        ...
        for (NSString *settingName in [settingsGroup allKeys]) {
            ...
            STHTSettings *settingToCheck = [[[self currentSettings] filteredArrayUsingPredicate:predicate] lastObject];

            NSString *settingValue = [settingsGroup valueForKey:settingName];
            
            if ([[self.startSettings allKeys] containsObject:settingName]) {
                NSString *nValue = [STHTSettingsController normalizeValue:[self.startSettings valueForKey:settingName] forKey:settingName];
                if (nValue) {
                    settingValue = nValue;
                } else {
...
- (NSString *)applyNewSettings:(NSDictionary *)newSettings {
    ...
    for (NSString *settingName in [newSettings allKeys]) {
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF.name == %@", settingName];
        STHTSettings *setting = [[[self currentSettings] filteredArrayUsingPredicate:predicate] lastObject];
        value = [STHTSettingsController normalizeValue:[newSettings valueForKey:settingName] forKey:settingName];
        if (value) {
            setting.value = value;
        } else {
            value = setting.value;
...
//    NSLog(@"controller didChangeObject");
    ...
    if ([anObject isKindOfClass:[STHTSettings class]]) {
